...
const ModalCargaMasiva = ({ abierto, onCerrar, onAplicado }) => {
  ...
  const handleArchivo = async (e) => {
    ...
    try {
      ...
      const filas = raw.map(normalizar).filter((r) => r.sku || r.name);
      setFilas(filas);
      if (!filas.length) {
      ...
      const { data } = await api.post("/admin/productos/preview", { productos: filas });
      setPreview(data);
    } catch (err) {
  ...
  const aplicar = async () => {
    setAplicando(true);
    try {
      const { data } = await api.post("/admin/productos/aplicar", { productos: filas });
    ...
    <div className="fixed inset-0 z-40 flex items-center justify-center p-4" role="dialog" aria-modal="true">
      ...
      <div className="relative w-full max-w-lg rounded-3xl bg-canvas p-6 shadow-sheet">
        <div className="mb-5 flex items-center justify-between">
          <h2 className="text-lg font-bold text-ink">Cargar productos</h2>
